...
router.get("/quizmain", function(req, res) {
  ...
  if (sess) {
    let data = fs.readFileSync(path.join(__dirname, "..", "/public/html/quizmain.html"), 'utf8');
    if(data){
      // console.log(sess.userDetails.firstname,"Hello *************************************");
    res.send(data.replace('{{name}}',sess.userDetails.firstname));}
